...
| Starters checklist (`#starters-checklist`) | opt-in | Print the published competitor list as the check-off sheet the recorder takes onto the committee boat: one table per start (a class scored under IRC and ECHO at once is one table, each boat once), sail numbers set large, a box beside each, columns so a club night fits on one page. Printable by anyone with the link, or from Preview without publishing. | The printed sheet: three class tables of large sail numbers with tick boxes, seen under print media. |
