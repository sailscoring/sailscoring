...
| Finish-sheet import (`#importing-finish-sheet`) | opt-in | Import a whole race's finish sheet from CSV or Excel — sail numbers, optional finish or elapsed times, optional codes, row order as crossing order — with a preview before it replaces the race. | Import preview dialog reporting finishers, coded entries, and replacements. |
