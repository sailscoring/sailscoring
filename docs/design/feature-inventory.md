...
| Feature (help §) | Status | Description | Screenshot |
|---|---|---|---|
| One-click publish (`#publishing-results`) | core | Publish standings to a stable public URL under your workspace — an explicit point-in-time action, with the dialog counting edits since the last publish. A page's URL is fixed once published, so links to it keep working — but a single page can still be moved to a new one, coming down and going back up on its own while every other page of the series stays live. | Publish dialog over a multi-fleet series: fleet checkboxes, editable URL segments, edits-since note. |
