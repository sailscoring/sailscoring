...
| Starters checklist (`#starters-checklist`) | opt-in | Print the published competitor list as the check-off sheet the recorder takes onto the committee boat: one table per start (a class scored under IRC and ECHO at once is one table, each boat once), sail numbers set large, a box beside each. Built to be written on — blank ruled space on every row for the number a boat actually turned up under, three spare rows per start for a late entry, and a Notes block at the end — paid for by dropping the logos and headings in favour of one line with Date, Race(s) and Recorder left blank. Printable by anyone with the link, or from Preview without publishing. | The printed sheet: class tables of large sail numbers with tick boxes and writing space, seen under print media. |
