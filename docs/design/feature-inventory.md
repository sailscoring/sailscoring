...
| Competitor list and fields (`#adding-competitors`) | core | Each entry carries a sail number and name plus whichever optional fields the series enables — boat, class, owner, helm, crew, club, gender, age, bow number, entry number, tally number, nationality (with flags), and prize-giving subdivision axes. Enabled fields that carry values are published as columns on results pages. | Competitors tab of a keelboat series showing nationality flags and a Division column. |
