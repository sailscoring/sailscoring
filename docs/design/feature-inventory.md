...
| Competitor list and fields (`#adding-competitors`) | core | Each entry carries a sail number and name plus whichever optional fields the series enables — boat, class, owner, helm, crew, club, gender, age, bow number, entry number, tally number, nationality (with flags), and prize-giving subdivision axes. A boat can hold more than one club — the standard Irish entry sheet's Club and Other Club both survive the import, and every club is published. Enabled fields that carry values are published as columns on results pages. | Competitors tab of a keelboat series showing nationality flags and a Division column. |
| Excluded competitors (`#excluded-competitors`) | core | A boat on the list that has not entered can be excluded rather than deleted: it is scored nowhere, sits on no published page, and does not count toward the entry total that DNC and DNF points are based on. Sailwave's Exclude flag under the same name, imported with it; the finish sheet groups excluded boats apart and offers to include one that turns up. In a series with sub-series a boat can be included in or excluded from one block alone, and the Standings tab lists every boat a table leaves off under "Not shown". "Rank only boats that took part" on the Scoring card applies the rule automatically: a boat with no result yet is not an entrant until it sails. | Competitors tab with one row's Excluded box ticked and the row muted, the header reading "… · 1 excluded". |
...
| Race conditions and management team (`#race-management-metadata`) | opt-in | Record what a race was sailed in (wind range and direction, course notes) and who ran it, using World Sailing's role names — or a role you write out yourself for a job the manual doesn't name — publishable only by explicit choice. | Race record dialog with wind range, direction, and a small named team. |
...
| Split-fleet championships (`#split-fleets`) | operator | Run a qualifying/final championship — Yellow/Blue fleets re-dealt from the ranking each morning, Gold/Silver finals, combined interleaved finish sheets, catch-up and medal races — from a guided tab that restates your configuration as sailing-instruction prose, marking the sentences whichever setting you are on writes. Where the seeding committee hands over the assignment already made rather than an order to deal from, import it as a column on the entry list and assign the first round straight from it. Your event's own words and its own race labels are settings: the stage names your sailing instructions use, and what the notice board calls the races — QP1, Q1 or E1 — since championships under one class's instructions post them differently, and the label is what a competitor names on a scoring enquiry. The published standings carry that same prose, folded away, so a competitor reading the results can see how the event is scored. | Two shots: the Split Fleets tab of the seeded championship showing round assignments; the published championship standings with tiered Gold/Silver tables. |
...
| Feature (help §) | Status | Description | Screenshot |
|---|---|---|---|
| Finish entry (`#entering-results`) | core | The entry screen is a digital finish sheet — row order is crossing order, a sail number commits on Enter as soon as it's unambiguous, and timed fleets prompt for a finish time. A "Last in" line under the input names the boat just recorded and the place it took, so a scorer transcribing a paper sheet can confirm the entry without looking away from the input, and clicking it flashes the row. | Exists (`finish-entry.webp`): entry screen mid-race with finishers recorded and the Did-not-compete group below. |
...
| Scoring penalties (`#entering-results`) | core | Finishers penalised after a hearing take ZFP, SCP, or DPI additive penalties scored per RRS 44.3(c) and A6.2, shown in amber in the standings. A DPI can carry a short note of what it was for — "TPO" for a missed tally — published in place of the code with a line beneath the table explaining it. | Penalty editor open from a finisher's row-actions menu, with an amber penalised score like 4 (ZFP) visible in the standings. |
...
| Elapsed-time recording (`#elapsed-times`) | core | A finish boat working off a stopwatch records a duration per boat rather than a time of day; set the race to elapsed times and the time column takes them as written. Handicap scoring and the protest time limit behave identically either way. | A finish sheet in elapsed mode: four boats with durations in the time column and the recording-mode control reading Elapsed times. |
| Finish-sheet import (`#importing-finish-sheet`) | opt-in | Import a whole race's finish sheet from CSV or Excel — sail numbers, optional finish or elapsed times, optional codes, row order as crossing order — with a preview before it replaces the race. Where two one-design classes race together and both number from 1, the sheet's class column says which boat a shared sail number is. | Import preview dialog reporting finishers, coded entries, and replacements. |
| RaceSense import (`#racesense-import`) | operator | Read a whole regatta out of the race committee's Vakaros RaceSense export: every sheet matched to its race and marked new, unchanged or differing, so a day's results land in one upload while corrections made by hand are never overwritten. Boats over the line keep their OCS, which the finish list alone would have lost, and every race says how its start went — the boats that came to the line, the ones that were over and stayed over as a count and a share of the fleet, and the ones that were over and got back, who reach no other record at all. A boat whose device never checked in and never saw the start line is read as DNC rather than the DNF the export writes for every non-finisher alike — every boat whose code the import chose this way is named against her race for checking against the committee's record. Each finisher is imported on her elapsed time, which is what the device measured — the timestamp beside it is derived from that and has been seen going an hour wrong for individual boats, so it is cross-checked and reported rather than trusted. A finish that reads both short of the fleet's distance and ahead of its time is flagged for checking against the committee's results: that is what a boat whose crossing on an earlier lap was taken for her finish looks like, and it has put a boat first who really placed 25th. The export's track data — distance sailed, max speed, distance to line at the gun — is kept too, and an opt-in publish setting puts it on the per-race tables as sortable columns, average speed derived. The scorer sees it without publishing anything: the import counts what each sheet captured, the races list badges every race with the boats it holds data for, and a marker on each finish row opens that boat's figures underneath. The same record can be read without waiting for the export: paste the regatta's replay address from the RaceSense player and the app reads the committee's document behind it — the same starts, OCS calls and finishes, read the same way — so a race is imported the moment it finishes and the end-of-day export confirms it Unchanged; a race still on the water is listed, not imported. | The import dialog over the sample league: two races confirming what's already entered as Unchanged, the third disagreeing with its three changed boats listed and deliberately ticked. |
...
| Fixed club handicap (`#fixed-tcf`, `#updating-handicaps`) | core | A handicap the club sets itself and holds for the whole series, applied time-on-time exactly as a TCC is — the shape of a club league whose numbers are set in advance from recent form rather than recalculated race by race. The fleet carries the club's own name for the number ("HPH"), which heads the rating column wherever it is shown and published, and the numbers can be seeded straight from the end-of-series handicaps of the progressive series they came from. A boat re-rated mid-series keeps its earlier races on the rating they were sailed under. | The Fleets card of a club league: a fleet scored on Fixed TCF with HPH typed in the label box beside the system picker. |
...
| ORC (`#scoring-orc`, `#update-handicaps-orc`) | opt-in | Whole ORC certificates imported from the ORC database (per country and family — a boat with no certificate in its fleet's family is rated off its standard one, marked as such — with expiry and VPP-year checks and CDL/GPH class-division columns), scored on any published single number — all-purpose ToT behaves exactly like IRC — or time-on-distance over a recorded course length, or performance curves (PCS) over the standard models or a constructed course entered leg by leg, or that same constructed course scored at the wind the race committee recorded on each leg instead of at one derived from the finish times — the rating computed from the course geometry and the observed wind, applied time-on-time or time-on-distance (ORC Scorer's "ToT / ToD - Constructed"), with no implied wind claimed because nothing is inferred from how the boats sailed. A race whose course has not been entered yet is left unscored rather than ranked on crossing order — a dash in the standings, out of every total and discard — and publishing holds the pages carrying that fleet, naming the race, until the course is in or the scorer publishes without it. Neither Sailwave nor HalSail scores PCS or constructed courses. The scoring options are named the way the certificate names them — "5-Band All Purpose L/M", not `IRL_AP_LM_TOT` — and grouped by the issuing office, from the certificates' own catalog; since a certificate carries every country's national fields, only the options that catalog names are offered, rather than the several hundred belonging to offices on other continents. | Published PCS race table from the seeded ORC sample: the audit header (constructed course, scoring wind and its source, scratch allowance), the ToD and Implied wind columns, and the leg-by-leg course record. Also `orc-recorded-wind`: the sample's same course scored at the recorded wind, framed under the performance-curve race so the two read against each other. |
| Course builder (`#course-builder`) | opt-in (with ORC) | A Courses tab on an ORC series: a mark library (the club's charted marks adopted from its course card at `courses.sailscoring.ie`, and the line, finish and windward marks the race committee laid, entered as coordinates or as a bearing and distance off another mark) and a course library (a number on the card, with the dialog naming exactly the marks the card cannot place, built by hand from the marks, or — where the committee boat handed over legs and no positions — the leg table itself, typed or pasted straight from the committee's own table, which is drawn from its bearings and distances alone and reports how nearly it closes). A race start picks a course and its legs fill in at the start's wind; edits to the legs are kept and flagged, and the start keeps a snapshot drawn on the published page. Every dialog draws the marks and legs as they are typed. | The Courses tab of the seeded ORC sample: the marks from the card and the laid ones, the course built from card J2, and the drawing. |
| ORC handicap mix (`#scoring-orc`) | opt-in (with ORC) | Beside the course on a published performance-curve race, folded away: the certificate's own allowance table, laid out as it is printed, with the share of the rating each cell actually carried in place of the allowance — half beat VMG and half run VMG for a windward/leeward model, the angles the legs were sailed at for a constructed course, every row for an all-purpose one, and a wind axis showing that PCS reads one scoring wind where a single-number rating averages over a fixed spread. Says whether the weights reproduce the applied allowance or only attribute it. Nothing comparable is published by any other scoring tool. | The handicap-mix grid open under the constructed-course race of the seeded ORC sample. |
| Carrying handicaps between series (`#updating-handicaps`) | core | Carry every boat's handicap forward from a prior series — end-of-series TCFs for progressive systems, and those same TCFs pinned as the fixed numbers of a club league — with a full current → new preview and a mid-series-vs-correction choice for already-scored races. | Update handicaps dialog, prior-series source, with the keep-already-scored-races-on-the-old-rating checkbox visible. |
| Rating transparency (`#rating-systems`) | core | Progressive fleets always show each boat's next rating, and a toggle reveals the full per-race calculation columns so a rating officer can verify every adjustment by calculator. An IRC rating applied from the list keeps the certificate it came from — number, the certificate's own sail number and boat name, hull dimensions, and which listing and when — shown side by side with the entry wherever the match rested on more than the sail number, and named on the published page — the listing and its date in a footnote, the certificate on each boat's own rating cell — so a competitor can trace a rating without writing to the scorer. | Published NHC or ECHO race table with the rating-calculation columns revealed. |
...
| Feature (help §) | Status | Description | Screenshot |
|---|---|---|---|
| One-click publish (`#publishing-results`) | core | Publish standings to a stable public URL under your workspace — an explicit point-in-time action, with the dialog counting edits since the last publish. Publish sits in the series header beside the ⋯ menu, so it is one click away from every tab, including the race you have just finished entering. A page's URL is fixed once published, so links to it keep working — but a single page can still be moved to a new one, coming down and going back up on its own while every other page of the series stays live. | Publish dialog over a multi-fleet series: fleet checkboxes, editable URL segments, edits-since note. |
...
| Race grid (`#combined-pages`) | opt-in | Open a full-detail combined page's race tables one at a time from a grid of fleets and races, instead of scrolling a league's worth of them. The page still carries every table, so it prints whole and a cell's link can be sent to a class captain. | Published Overall page led by the grid, standings beneath. |
...
| Published competitor list (`#competitor-list`) | opt-in | Publish the entry list before racing starts — who is coming, with the fields the series records, and nothing derived from results. It is the one page a series with no races yet can publish, which is exactly the window an event most wants one. Where a class is scored under two systems at once the page is tabled by class, each table carrying a rating column per fleet — the boat is one row, its ratings say which fleets it is in, and a blank column cell is a certificate still to come. | Exists (`published-competitor-list.webp`): a league's entry list tabled by class, each table with an IRC and an ECHO column. |
| Starters checklist (`#starters-checklist`) | opt-in | The published competitor list carries the check-off sheet the recorder takes onto the committee boat, as a view with an address of its own — look at it before you print it, and print it with your browser's ordinary Print, a phone's share sheet included: one table per start (a class scored under IRC and ECHO at once is one table, each boat once), a box to tick down the left edge, sail numbers set large and right-aligned against the boat name, a class that runs into the next column carrying its name over. Built to be written on — blank ruled space on every row for the number a boat actually turned up under, two spare rows per start for a late entry, and a Notes block at the end — paid for by dropping the logos and headings in favour of one line with Date, Race(s) and Recorder left blank. Printable by anyone with the link, or from Preview without publishing. | The sheet: class tables with tick boxes down the left, large sail numbers and writing space, seen under print media. |
| Notes on published pages (`#page-notes`) | opt-in | Put a sentence of explanation on a published page — why a fleet assignment deliberately does not reconcile with the standings beside it, that a race was abandoned and resailed, that the finish order was corrected at 16:40, how a reconstructed archive differs from the originals it links to. One note per page and one on every page of a publication, plain text with links, written in Preview with the page in view or from a row of the Publish dialog, and printed above the results as an editorial aside. | Preview with the note strip above a fleet-assignments page, and the note rendered in the page below it. |
...
| FTP upload to your own site (*inline in* `#publishing-results`) | operator | Push published results pages directly onto your club's own web hosting over FTP or FTPS, with a remote path per published page remembered per series; each upload is pinned in the series history like an in-app publish. | Publish dialog in Your website (FTP) mode: server selected, per-page remote paths with checkboxes. |
...
| Fleet planning on import (`#importing-competitors`) | core | Decide the fleets before the columns: pick what splits the boats, see the fleets the ratings imply, then rename, re-scope, drop, or add one the spreadsheet says nothing about — an IRC fleet with certificates still to come. An entry list can't say who holds a certificate, so the import ends where that becomes knowable: it names how many boats in which fleet have no rating yet and offers the rating list, which fills in the ratings it has and offers the boats it doesn't rate for removal from the fleet. | The importer's Fleets step: a group with an NHC fleet and an added IRC one, showing boat counts. |
...
| Sailwave export (`#sailwave-export`) | opt-in | Download any series as a Sailwave `.blw` — fleets and ratings, entries, starts, finishes and codes, the discard profile — so a scorer wary of depending on the app has Sailwave as a fallback at any moment. Sailwave re-scores from the inputs; whatever it cannot carry is listed before the download. | The Export to Sailwave dialog listing what the Sailwave copy will not carry. |
...
| Open in Sail Scoring, and the data behind published results (`#json-export`) | core | Every publication serves its results data as a `.sailscoring.json` file beside the pages (ADR-012), linked from every footer. Anyone — no account needed — can open a published event in the app and read it: entry list, racing, standings, and how it was scored. Saving a copy signs them in and imports it as their own series to re-score or experiment with. Standalone downloads embed the snapshot instead. | The read-only spectator view of a published event (banner over real standings, signed out), and the published-page footer with its data-file link. |
...
| Feature (help §) | Status | Description | Screenshot |
|---|---|---|---|
| Competitor identity and timelines (`#competitor-identity`) | operator | Entries across seasons resolve into recurring competitors, each with a public timeline of every series sailed ("3rd of 48") and a searchable competitor index. One club written several ways counts as one club, and where entries usually leave the club blank — a club scoring its own racing — a blank is read as its own members rather than as unknown. | Public competitor timeline spanning several seasons; competitor index with a search underway. |
...
| Workspace activity log (`#workspace-activity`) | core | The whole workspace's log on one tab — every series' changes and every time its results went out or came down, newest first, plus what happens to the workspace itself: a series deleted or purged, a support session joining and leaving — each entry attributed and linked to its series while it exists. | The workspace Activity tab with several series-linked entries and one workspace-level entry. |
...
| Searching help (`#searching-help`) | core | A search box over the help index, on the help page and in the panel, reached with `/`. It matches each section's own keywords as well as its title, so the words a scorer reaches for find their section even when no title contains them — DNC, RDG, TCF, nett, burgee, stopwatch — and the Sailwave and HalSail names for the same thing are in there too. Two words narrow rather than widen. Sections gated off for a workspace aren't listed and aren't searched. | The help index with "discard" typed and the list narrowed to the sections that cover it. |
