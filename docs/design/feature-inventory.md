...
| Rating transparency (`#rating-systems`) | core | Progressive fleets always show each boat's next rating, and a toggle reveals the full per-race calculation columns so a rating officer can verify every adjustment by calculator. An IRC rating applied from the list keeps the certificate it came from — number, the certificate's own sail number and boat name, hull dimensions, and which listing and when — shown side by side with the entry wherever the match rested on more than the sail number, and named on the published page so a competitor can trace a rating without writing to the scorer. | Published NHC or ECHO race table with the rating-calculation columns revealed. |
