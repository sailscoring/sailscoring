...
| Published competitor list (`#publishing-results`) | opt-in | Publish the entry list before racing starts — who is coming, with the fields the series records, and nothing derived from results. It is the one page a series with no races yet can publish, which is exactly the window an event most wants one, and Publish sits on the Competitors tab as well as Standings. | Published competitor list: a multi-fleet entry list with tally numbers and nationality flags. |
