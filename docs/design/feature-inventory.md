...
| Starters checklist (`#starters-checklist`) | opt-in | Print the published competitor list as the check-off sheet the recorder takes onto the committee boat: one table per start (a class scored under IRC and ECHO at once is one table, each boat once), a box to tick down the left edge, sail numbers set large and right-aligned against the boat name, a class that runs into the next column carrying its name over. Built to be written on — blank ruled space on every row for the number a boat actually turned up under, two spare rows per start for a late entry, and a Notes block at the end — paid for by dropping the logos and headings in favour of one line with Date, Race(s) and Recorder left blank. Printable by anyone with the link, or from Preview without publishing. | The printed sheet: class tables with tick boxes down the left, large sail numbers and writing space, seen under print media. |
