...
| JSON export and Open in Sail Scoring (`#json-export`) | core | Every publication serves its results data as a `.sailscoring.json` file beside the pages (ADR-012) — linked from every footer with an Open in Sail Scoring link that imports the whole series into any account, one click. Standalone downloads embed the snapshot instead. | Published-page footer with the Open in Sail Scoring and data links, and the import confirmation with its workspace picker. |
