...
| ORC (`#scoring-orc`, `#update-handicaps-orc`) | opt-in | Whole ORC certificates imported from the ORC database (per country and family, expiry and VPP-year checks, CDL/GPH class-division columns), scored on any published single number — all-purpose ToT behaves exactly like IRC — or time-on-distance over a recorded course length, or performance curves (PCS) over the standard models or a constructed course entered leg by leg. Neither Sailwave nor HalSail scores PCS or constructed courses. | Published PCS race table from the seeded ORC sample: the audit header (constructed course, scoring wind and its source, scratch allowance), the ToD and Implied wind columns, and the leg-by-leg course record. |
