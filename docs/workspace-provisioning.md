...
| `sub-series` | off | the **Add sub-series** button and the per-race **scissors** (split) control on a series' **Races** tab, #203: divide one series into named blocks of races (e.g. a Frostbite Winter + Spring), each scored independently — its own standings (a block selector on the **Standings** tab), discards (the series discard rule applied to each block's race count), entrants (a boat absent from a block isn't in it), and published page (`/p/{ws}/{series}/{block}/{fleet}`). Only the *authoring* gestures are gated: a series that already has sub-series renders its blocks, scores them, and publishes them regardless. Progressive handicaps chain across block boundaries unchanged. Off by default until the model is proven against a real season. |
| `competitor-identity` | off | the cross-series **competitor-identity** spine (#212): the workspace **Identities** reconcile surface that collapses a sailor's per-series competitor rows onto one recurring identity (`competitors.identity_id`), and the public **career-arc** page read off that link (every series a sailor entered, their results and ranking over the years). Off by default and introduced for IODAI first — a one-design junior class whose ~180-series corpus back to 2009 makes the career arc the showcase. Invisible and inert in every other workspace. Identity is workspace-local: excluded from the `.sailscoring` file format and public JSON export, re-derived by the reconcile pass. |
