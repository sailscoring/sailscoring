...
| `split-fleets` | off | The **Split Fleets** tab on a series (docs/design/split-fleets.md): the guided qualifying/final championship workflow — seed qualifying fleets, reassign by rank each day, split into Gold/Silver/Bronze, medal races, publish the championship standings + rolling assignment lists. Creates ordinary fleets/races/starts, so data survives the gate being turned off. Off by default and not self-service: enable per-workspace for championship events; no GA until the format has been validated against a real event. |
