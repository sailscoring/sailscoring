...
| `sailwave-export` | off | **Export to Sailwave…** on the series ⋯ menu (and its `S` shortcut), #516: downloads the series as a Sailwave `.blw` — fleets, entries, ratings, starts, finishes and codes, the discard profile — so a scorer wary of depending on the app has Sailwave as a fallback at any moment. Only scoring inputs travel and Sailwave rescores from them; anything Sailwave cannot express (sub-series, split-fleet rounds, per-race discard policies and multipliers, custom NHC parameters, ORC) is listed before the download. Off by default: it is an adoption aid we may retire, so the audience stays known. |
...
| `combined-pages` | off | the **Combined pages** card on a multi-fleet series' **Settings** tab and the combined-page rows in the **Publish** dialog (#255): publish several fleets' results as sections of **one page** — an all-fleets "Overall" page (typically standings only), or a multi-method class page (e.g. one "Puppeteer" page carrying its Scratch and HPH fleets in full detail); a single series-level toggle ("Publish individual per-fleet pages") switches the standalone fleet pages off so the combined pages **replace** them. Also unlocks the per-page **Show a race grid** option (#604), which opens a full-detail page's race tables one at a time from a grid of fleets and races. Only the *authoring* UI is gated: a series that already carries group config keeps rendering and publishing it, like `sub-series`; when the feature is off, a series that carries such config shows a hint on its **Settings** tab pointing at this toggle (#280). Off by default while the page composition proves out. |
...
| `page-notes` | off | an explanatory **note on a published page** (#511): free text a scorer writes in Preview (where the page is visible) or from a row of the Publish dialog, rendered above the results as an editorial block. Two slots — one note on every page of a publication, one per page — with plain text, `[label](url)` links and bare URLs, and nothing else. Notes travel in the `.sailscoring` file (v46) and in the published `.sailscoring.json`, so an import and the spectator viewer keep them. For the cases figures cannot state themselves: a fleet assignment that deliberately does not reconcile with the standings, "corrected 16:40 — Q1 finish order revised", a reconstructed archive's stated delta with a link to the originals. Off by default while the shape proves out; a workspace with it off publishes exactly what it published before. |
