...
| `split-fleets` | off | **PROTOTYPE** — the **Split Fleets** tab on a series (docs/design/qualifying-final-series.md): the guided qualifying/final championship workflow — seed qualifying fleets, reassign by rank each day, split into Gold/Silver/Bronze, medal races — plus the `/api/v1/series/:id/split-fleets` endpoints behind it. Creates ordinary fleets/races/starts, so data survives the gate being turned off. Off by default; throwaway prototype for design review, do not enable for real workspaces. |
...
| `split-fleets` | prototype under review; expert machinery, audience stays enumerable |
