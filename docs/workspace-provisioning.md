...
| `competitor-reconcile` | off | the **in-app** reconcile surface (#212, #217): the **Competitors** entry in the workspace switcher and the `/workspace/competitors` page (rename / split the auto-matched identities), plus the `/api/v1/competitor-identities` endpoints behind it. Separate from `competitor-identity` so the public competitor pages can be live while the in-app rename/split UI stays hidden — the reconcile UX isn't settled and cleanup currently runs out-of-band (the iodai-archive manifest, #218). Off by default; leave it off until the reconcile UI is ready (tracked in #221). |
