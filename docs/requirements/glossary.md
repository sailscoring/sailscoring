...
| Repêchage | A second-chance round: boats that missed the direct cut into a later stage race again, among themselves, for the places still open in it. The word is from rowing and cycling (French *repêcher*, "to fish out again") and appears nowhere in the RRS or Appendix LE. The Irish Sailing Dinghy and Keelboat Champions' Cups sail one between their qualifying flights and their final series, scored on its own races alone. |
...
| Scoring wind | The single wind speed a race's performance curves are read at: under PCS the winning boat's implied wind (rule 402.9), unless the race committee overrides it as unrepresentative (402.12); under the recorded-wind options the wind the committee measured, distance-weighted across the legs. Each boat's time allowance at the scoring wind, read off its own performance curve, becomes that race's rating. |
| Recorded wind | The true wind speed the race committee measured on a leg, entered as a scoring input alongside its direction. What separates the recorded-wind constructed-course options from PCS, which derives the wind from the finish times instead and so records none. |
| Recorded-wind constructed course | Scoring a constructed course at the recorded wind: each boat's performance curve is built from its certificate over the race's legs, exactly as for PCS, and read at the wind the committee recorded rather than at one implied by the race. The resulting allowance is applied time-on-time (600 / allowance) or time-on-distance. ORC Scorer calls these "ToT - Constructed" and "ToD - Constructed"; the rating is on no certificate — it belongs to that race. No boat has an implied wind under these options, since nothing is inferred from how the boats sailed. |
...
| Scoring option | The method one race is scored under — a certificate single number (APHT, a W/L or coastal number), a wind band, or performance curves (a course model or a constructed course). A per-fleet-per-race race-committee choice: the SIs name a default, and the committee may announce a different option for a given race (a coastal day scored ToD in a series otherwise on curves, a band when one is called). In Sail Scoring the fleet setting is the default and each race start can name the race's option. |
| Wind band | One entry of a banded rating set (triple-number or a national multi-band option): a wind-speed range mapped to a pre-computed rating. Band selection is a per-race race-committee decision — announced, and changeable if conditions materially change — not series configuration. A band is one kind of scoring option; picking it per race is the same mechanism as switching method per race. |
...
| Course (library) | A named course a race start picks, defined one of two ways: a sequence of the series' marks, from which distances and bearings are derived so a corrected mark corrects every course built on it; or the race committee's own leg table, where the legs *are* the course and there are no positions behind them — which is all a committee boat often hands over, and the only thing a course is in ORC Scorer. Neither carries the wind: the same course sails on a different night. |
