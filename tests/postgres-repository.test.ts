...
describe.skipIf(skip)('postgres repositories', () => {
  ...
  test('CompetitorRepository: optional fields, fleetIds[] round-trip; sorted by sail number', async () => {
    ...
    const c1: Competitor = {
      ...
      crewName: 'Crew', club: 'HYC', gender: 'M', age: 42,
      createdAt: Date.now(),
  ...
  test('TcfRecordRepository.listBySeries returns records joined through races, workspace-scoped', async () => {
    const reposA = createRepos({ db, workspaceId: workspaceA });
    const reposB = createRepos({ db, workspaceId: workspaceB });
    ...
    await reposA.series.save(s);
    const fleet = uuid();
    await reposA.fleets.save({
      id: fleet, seriesId: s.id, name: 'NHC', displayOrder: 0, scoringSystem: 'nhc',
    });
    const c1: Competitor = {
      id: uuid(), seriesId: s.id, fleetIds: [fleet],
      sailNumber: 'A', name: 'A', club: '', gender: '', age: null,
      createdAt: Date.now(), nhcStartingTcf: 1.0,
    };
    const c2: Competitor = {
      id: uuid(), seriesId: s.id, fleetIds: [fleet],
      sailNumber: 'B', name: 'B', club: '', gender: '', age: null,
      createdAt: Date.now(), nhcStartingTcf: 1.0,
    };
    await reposA.competitors.save(c1);
    await reposA.competitors.save(c2);
    const r1: Race = { id: uuid(), seriesId: s.id, raceNumber: 1, date: '2026-04-01', createdAt: Date.now() };
    const r2: Race = { id: uuid(), seriesId: s.id, raceNumber: 2, date: '2026-04-08', createdAt: Date.now() };
    await reposA.races.save(r1);
    await reposA.races.save(r2);

    // Seed directly — the scoring engine owns writes, but for this read-path
    // test we just need rows in the table.
    await db.insert(schema.tcfRecords).values([
      { id: uuid(), raceId: r1.id, competitorId: c1.id, fleetId: fleet, tcfApplied: 1.0, newTcf: 1.01 },
      { id: uuid(), raceId: r2.id, competitorId: c1.id, fleetId: fleet, tcfApplied: 1.01, newTcf: 1.02 },
      { id: uuid(), raceId: r1.id, competitorId: c2.id, fleetId: fleet, tcfApplied: 1.0, newTcf: 0.99 },
    ]);

    const list = await reposA.tcfHistory.listBySeries(s.id);
    expect(list).toHaveLength(3);
    expect(new Set(list.map((r) => r.competitorId))).toEqual(new Set([c1.id, c2.id]));

    // Workspace B sees nothing: the join through races filters on
    // workspace_id, so a different workspace can't read this series's
    // history even if it guesses the series id.
    const listFromB = await reposB.tcfHistory.listBySeries(s.id);
