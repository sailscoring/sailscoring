...
function makeRace(id: string, n: number, subSeriesId: string | null): Race {
  return { id, seriesId: 's1', raceNumber: n, date: '2026-11-01', createdAt: 0, subSeriesId };
}
...
const winter: SubSeries = { id: 'ss-w', seriesId: 's1', name: 'Winter', displayOrder: 0 };
const spring: SubSeries = { id: 'ss-s', seriesId: 's1', name: 'Spring', displayOrder: 1 };

const snapshot: SeriesSnapshot = {
  series: makeSeries('s1'),
  competitors: [makeCompetitor('c1', '101'), makeCompetitor('c2', '102')],
  fleets: [fleet],
  races: [makeRace('r1', 1, 'ss-w'), makeRace('r2', 2, 'ss-w'), makeRace('r3', 3, 'ss-s')],
...
describe('.sailscoring v9 sub-series round-trip', () => {
  it('buildSeriesFile carries blocks and race membership', async () => {
    ...
    expect(file.subSeries).toEqual([
      { id: 'ss-w', name: 'Winter', displayOrder: 0 },
      { id: 'ss-s', name: 'Spring', displayOrder: 1 },
    ]);
    expect(file.races.map((r) => r.subSeriesId)).toEqual(['ss-w', 'ss-w', 'ss-s']);
  ...
  it('a blockless series writes no subSeries key', async () => {
    const blockless: SeriesSnapshot = {
      ...snapshot,
      subSeries: [],
      races: snapshot.races.map((r) => ({ ...r, subSeriesId: null })),
    };
    const { repos } = makeRecordingRepos(blockless);
    const file = await buildSeriesFile('s1', repos);
    expect(file.subSeries).toBeUndefined();
    expect('subSeriesId' in file.races[0]).toBe(false);
  });

  it('openSeriesFromFile rebuilds blocks with fresh ids and keeps membership', async () => {
    ...
    expect(savedRaces.map((r) => r.subSeriesId)).toEqual([
      newWinter.id, newWinter.id, newSpring.id,
    ]);
  });
...
describe('public JSON export sub-series round-trip', () => {
  it('export names each race block; import rebuilds blocks in race order', async () => {
    ...
    const idByName = new Map(savedSubSeries.map((ss) => [ss.name, ss.id]));
    expect(savedRaces.map((r) => r.subSeriesId)).toEqual([
      idByName.get('Winter'), idByName.get('Winter'), idByName.get('Spring'),
    ]);
  });

  it('a blockless export carries no subSeries keys', () => {
    const blockless: SeriesSnapshot = {
      ...snapshot,
      subSeries: [],
      races: snapshot.races.map((r) => ({ ...r, subSeriesId: null })),
    };
    const data = buildPublicExportFromSnapshot(blockless);
...
describe('buildFleetHtmlFiles with sub-series', () => {
  ...
  it('a blockless series still renders one page per fleet', async () => {
    const { buildFleetHtmlFiles } = await import('@/lib/results-export');
    const blockless: SeriesSnapshot = {
      ...snapshot,
      subSeries: [],
