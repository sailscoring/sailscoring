...
- ADR-011: Public results navigation and the publication tree (Accepted — `/p/` URLs reinterpreted as workspace → folder (usually a season) → folder → page with no URL changes; interior index pages, first-class workspace-defined seasons, category as a facet, one sibling-based navigation cascade on every public page)
