...
- ADR-012: The data behind a published results page (Accepted, amended 2026-08-30 — publishing a page also publishes its sanitized public export beside it as a snapshot-pinned `.sailscoring.json` sidecar in a stable, versioned, documented format; the embedded base64 blob goes away for published pages; unpublish deletes the sidecar; built toward a no-account, strictly read-only spectator mode — "what if" experimenting happens on an imported copy after sign-in, never in the viewer)
