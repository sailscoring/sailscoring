...
See `docs/` for design docs, ADRs, requirements, and glossary. External third-party source material that isn't part of the codebase — the RRS, NORs / Sailing Instructions for target events, handicap-system rules (ECHO, RYA NHC, ORC), and manuals for comparable tools (Sailwave, HalSail, ORC, ZW, CORK) — lives out-of-band in the `reference-docs` sibling repo (see the table below); the old in-repo `reference/` directory has been removed. **These are now searchable markdown** (not just PDFs), so when designing a feature, consult `../reference-docs` for how the rules, events, and comparable tools actually work — grep it for the relevant terms before reinventing the domain logic. Build inputs that the app actually generates data from live under `scripts/data/` (RYA PY sources, the frozen sample-series rating seed). New design notes go in `docs/design/`.
...
| `sailscoring/reference-docs` | `../reference-docs` | **Third-party reference docs** (RRS, handicap-system rules, scoring-tool manuals, event NoRs/SIs), relocated out of this repo's old `reference/` directory. Now **searchable markdown** (converted from the source PDFs), bucketed into `rrs/ handicap-systems/ tool-manuals/ events/`, with a README mapping each to its source. The app reads none of it, but it's a rich design reference for how the rules, target events, and comparable tools (Sailwave, HalSail, ORC, ZW, CORK) actually work — grep it before reinventing domain logic. |
