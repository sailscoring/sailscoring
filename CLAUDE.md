...
- ADR-012: The data behind a published results page (Accepted — publishing a page also publishes its sanitized public export beside it as a snapshot-pinned `.sailscoring.json` sidecar in a stable, versioned, documented format; the embedded base64 blob goes away for published pages; unpublish deletes the sidecar; built toward a no-account view-only mode with client-side "what if" scratch edits)
