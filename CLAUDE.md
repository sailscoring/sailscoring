...
| `sailscoring/reference-docs` | `../reference-docs` | **Third-party reference PDFs** (RRS, handicap-system rules, scoring-tool manuals, event NoRs/SIs), relocated out of this repo's old `reference/` directory. Plain PDFs bucketed into `rrs/ handicap-systems/ tool-manuals/ events/`, with a README mapping each to its original filename + source. Human reference only — the app reads none of it. |
