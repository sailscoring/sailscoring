...
- ADR-013: Removing meaning from stored formats (Proposed — readers never drop a version; one upgrader per change, called from every entry point; removed values classed lossless / presentational / scoring, and a scoring-class value is refused by name, never silently rescored; guarded migrations keep the DB at the current shape; first applied by #636)
