 ...
 * than right.
 *
 * Rows are kept in the order the scorer put them in rather than sorted by
 * seniority — a club series that puts the week's duty officer first should
 * stay that way, and every read path preserves the order too. Which is why
 * the rows drag: the order is the scorer's to state, so it has to be theirs
 * to correct without retyping the list.
 */
    ...
    <div className="space-y-2" data-testid={`${idPrefix}-officials`}>
      <SortableList
        ...
        onReorder={(orderedIds) =>
          onChange(orderedIds.map((id) => value.find((o) => o.id === id)!))
        }
      >
        {(official, { ref, style, handleProps }) => {
          const index = value.indexOf(official);
          return (
            <div
              key={official.id}
            ...
            >
              <DragHandle
                {...handleProps}
                aria-label={`Reorder team member ${index + 1}`}
                data-testid={`${idPrefix}-official-drag-${index}`}
              />
              <Select
                value={official.role}
