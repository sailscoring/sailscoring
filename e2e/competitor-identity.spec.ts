...
test.describe('home club (#507)', () => {
  test('names the club a blank entry belongs to, and remembers it', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', (m) => m.type() === 'error' && errors.push(m.text()));
    page.on('pageerror', (e) => errors.push(e.message));

    const email = await signInFreshUser(page, 'homeclub');
    const { id: orgId } = await createOrgWorkspace('Home Club SC');
    await addMemberByEmail(orgId, email, 'owner');
    await enableOrgFeatures(orgId, ['competitor-identity']);
    await setActiveWorkspace(page, orgId);

    await page.goto('/workspace');
    const card = page.getByTestId('home-club-card');
    await expect(card).toBeVisible();

    const input = card.getByLabel('Home club');
    await expect(input).toHaveValue('');
    await input.fill('Killaloe Sailing Club');
    // The Save button is disabled both while the request is in flight and once
    // it lands, so waiting on the response is what actually says it saved.
    await Promise.all([
      page.waitForResponse(
        (r) => r.url().includes('/api/v1/workspace') && r.request().method() === 'PATCH',
      ),
      card.getByRole('button', { name: 'Save' }).click(),
    ]);

    // It is workspace state, not form state: a fresh load shows it filled in.
    await page.goto('/workspace');
    await expect(page.getByTestId('home-club-card').getByLabel('Home club')).toHaveValue(
