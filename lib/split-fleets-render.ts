...
export function renderSplitFleetStandingsPage(
  ...
  <td style="text-align:right;font-weight:bold">${row.net}</td>
        ...
        const cut = cuts.includes(i)
          ? `<tr><td colspan="${columns.length + 5}" style="border:none;padding:0"><div style="border-top:2px dashed #f59e0b;text-align:center;font-size:0.75em;color:#b45309;text-transform:uppercase">provisional split if qualifying ended now</div></td></tr>`
          : '';
        return tr + cut;
      })
      .join('\n');
      ...
      const fleets = round.fleetIds
        .map((fid) => {
          ...
          const rowsHtml = members
            .map(
              (c) =>
                `<tr><td style="font-family:monospace">${esc(c.sailNumber)}</td><td>${esc(c.names.join(' & '))}</td>${
...
<div class="wrap"><table><thead><tr><th>Sail</th><th>Helm</th><th></th></tr></thead><tbody>${rowsHtml}</tbody></table></div>`;
        })
        .join('\n');
