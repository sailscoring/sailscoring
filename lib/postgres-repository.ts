...
function seriesRowToType(row: SeriesRow): Series {
  return {
    ...
    proportionalDiscard: row.proportionalDiscard ?? undefined,
    dnfScoring: row.dnfScoring as Series['dnfScoring'],
    ...(row.excludeDncOnlyCompetitors ? { excludeDncOnlyCompetitors: true } : {}),
    raceFleetExclusions: row.raceFleetExclusions,
    ftpServerId: row.ftpServerId ?? undefined,
    ftpHost: row.ftpHost,
    ftpPath: row.ftpPath,
    ftpPaths: row.ftpPaths,
    ftpPagesExcluded: row.ftpPagesExcluded ?? undefined,
    publishMode: row.publishMode as Series['publishMode'],
    ftpLastUploadedAt: row.ftpLastUploadedAt ? row.ftpLastUploadedAt.getTime() : undefined,
    ftpUploadedVersion: row.ftpUploadedVersion ?? undefined,
    ...
    publishingGroups: row.publishingGroups,
...
function subSeriesRowToType(
  ...
  return {
    ...
    ...(row.competitorOverrides?.length ? { competitorOverrides: row.competitorOverrides } : {}),
    version: row.version,
  };
}
...
function seriesToRow(s: Series, workspaceId: string) {
  return {
    ...
    proportionalDiscard: s.proportionalDiscard ?? null,
    ...
    ftpServerId: s.ftpServerId ?? null,
    ftpHost: s.ftpHost,
    ftpPath: s.ftpPath,
    ftpPaths: s.ftpPaths,
    ftpPagesExcluded: s.ftpPagesExcluded ?? null,
    publishMode: s.publishMode ?? 'sailscoring',
    ftpLastUploadedAt: s.ftpLastUploadedAt != null ? new Date(s.ftpLastUploadedAt) : null,
    ftpUploadedVersion: s.ftpUploadedVersion ?? null,
    ...
    publishingGroups: s.publishingGroups ?? [],
...
const seriesUpdateColumns = [
  ...
  'ftpServerId', 'ftpHost', 'ftpPath', 'ftpPaths', 'ftpPagesExcluded', 'publishMode',
...
function subSeriesToRow(s: SubSeries, workspaceId: string) {
  return {
    ...
    excludeDncOnlyCompetitors: s.excludeDncOnlyCompetitors ?? false,
    competitorOverrides: s.competitorOverrides?.length ? s.competitorOverrides : null,
  };
}

const subSeriesUpdateColumns = [
  'name', 'displayOrder', 'fleetIds', 'startingHandicapSource', 'continueFromSubSeriesId',
  'excludeDncOnlyCompetitors', 'competitorOverrides',
